...
export default function LoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const res = await Fetch.postWithAccessToken<any>('/api/auth/signin', { email, password });
      if (res.data.code === Code.Success) {
        Cookie.set('access_token', res.data.data.token);
        router.push('/humanizer');
      } else if (res.data.code === Code.InactiveAuth) {
        router.push(`/wait-verify?email=${encodeURIComponent(res.data.data?.email || email)}`);
      } else {
  ...
  const handleGoogleSuccess = async (credentialResponse: any) => {
    try {
      const res = await Fetch.postWithAccessToken<any>('/api/auth/google', {
  ...
  return (
    <div className="bg-white rounded-2xl p-8 shadow-sm border border-rule">
      ...
      <p className="text-ink-muted mb-6">Sign in to continue to DoThesis</p>
