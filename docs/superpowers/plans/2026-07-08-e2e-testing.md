...
- Produces: `web/e2e/run-e2e.sh [playwright args…]` — the only supported way to run the suite; exports `DATABASE_URL`, `SESSION_SECRET`, `AWS_ENDPOINT_URL_S3`, `AWS_CONFIG_FILE` (forces S3 path-style addressing — see Note 4), `DOTHESIS_E2E_HAS_SOFFICE`.
...
- [ ] Export suite's download URL is path-style (`http://localhost:9123/dothesis-e2e/...`), not virtual-hosted (`http://dothesis-e2e.localhost:9123/...`) — confirms the `AWS_CONFIG_FILE` addressing-style fix (Note 4) actually took effect.
...
3. **`AWS_ENDPOINT_URL_S3` — VERIFIED, not just assumed** (upgraded after audit). None of the
   three s3 client factories (`api/app/s3.py:14`, `api/app/routers/uploads.py:42`,
   `orchestrator/tools/m5_writing.py:58`) pass an explicit `endpoint_url` kwarg, so botocore's
   env-var endpoint resolution isn't overridden by client code. The installed botocore
   (1.43.14 as of audit) is far past the ~1.29.x release that added
   `AWS_ENDPOINT_URL`/`AWS_ENDPOINT_URL_S3` support. No fallback needed.
4. **MinIO presigned-URL addressing style — FIXED, not just flagged** (audit found the
   originally-proposed fix, an `AWS_S3_ADDRESSING_STYLE` env var, does not exist in botocore:
   `DEFAULT_S3_CONFIG_VARS['addressing_style'] = (('s3','addressing_style'), None, None,
   None)` in `botocore/configprovider.py` — the `None` in the second slot means no env var is
   mapped to this config key). Task 1's bootstrap script now writes a scoped
   `AWS_CONFIG_FILE` with `[default]\ns3 =\n  addressing_style = path` instead — this is the
   real, documented botocore mechanism, applies process-wide (the bucket-create script and
   the uvicorn server both inherit it), and needs zero app code changes. **Verify in Task 7**;
   if MinIO still emits virtual-hosted URLs despite this, the remaining fallback is passing
   `Config(s3={"addressing_style": "path"})` explicitly at the three client-construction
   sites above, gated behind the same test-only env check.
...
8. **One `AIMessage` per completion in graph history** is the step-index premise. deepagents
   ...
   **Related, broader risk (audit finding):** `FakeChatModel.bind_tools()` is verified to
   satisfy `BaseChatModel`'s contract in isolation (Task 3's unit tests call
   `.invoke()`/`.astream()` directly), but the actual call site — LangChain's agent/middleware
   stack invoking `.bind_tools(tools, tool_choice=..., **model_settings)` inside a REAL
   `create_deep_agent` graph — is not exercised until Task 5 (onboarding) runs end-to-end.
   Task 3's green unit tests bound the model's *own* interface correctness; they do not close
   the *integration* risk. Treat Task 5's first real pass as the actual proof, not Task 3's.
...
14. **`needs_review` propagation is NOT exercised by this plan's suites (audit finding —
    genuine coverage gap, not a mocked shortcut).** Both `m1-to-m5.spec.ts` and the export
    suite's seed commit modules in strict forward canonical order, marking each `done`
    immediately — so every downstream module is still `locked` (the one status the
    `DOWNSTREAM` flagging loop in `agent/state.py:commit_slice` skips) at the moment its
    upstream neighbor commits, and the flip to `needs_review` can never fire under that
    pattern. **Follow-up scenario to close this** (out of scope for this plan's 9 tasks — add
    as Task 10 when picked up): after `m1-to-m5.spec.ts` reaches all-`done`, script one more
    fixture step that re-commits M1 (e.g. a revised research title) via
    `POST /test/seed-project` or a real chat turn, then assert `data-status="needs_review"`
    appears on the `ctx-M3`/`ctx-M4`/`ctx-M5` ContextPanel cards. This is a real, currently
    untested code path in production, not just in this harness — worth prioritizing.
