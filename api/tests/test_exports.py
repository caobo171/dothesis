...
# ---------------------------------------------------------------------------
# POST /projects/{pid}/export/module — the module write-up
# ---------------------------------------------------------------------------
...
def test_the_module_write_up_never_names_the_tool():
    """Nothing in a document a student hands in may name DoThesis.

    This route builds its docx with python-docx directly and never goes through
    run_export, so `results_render.scrub_sections` — the one place the invariant
    is enforced — could not see this pipeline at all. It printed "Module report ·
    generated by DoThesis" into a file the route itself calls teacher-ready.
    """
    from app.routers.exports import _build_module_docx

    text = _docx_text(_build_module_docx(
        "My project", "M3", "Research design",
        {"research_title": "Ý định sử dụng ví điện tử",
         "methodology": {"paradigm": "định lượng"},
         "hypotheses": ["H1: A → B"]},
    ))
    assert "dothesis" not in text.lower()
    assert "Module report" in text          # still labelled, just unbranded
    assert "Ý định sử dụng ví điện tử" in text


def test_the_module_write_up_still_renders_without_an_llm():
    """Composition is an LLM call and falls back to the structured renderer.
    The brand line sat above that branch, so both paths carried it."""
    from app.routers.exports import _build_module_docx

    text = _docx_text(_build_module_docx("P", "M1", "Topic", {}))
    assert "dothesis" not in text.lower()
