...
class BlogPost(Base):
    ...
    # What makes two posts the same article in different languages.
    #
    # Slugs are per-locale and a translated focus keyword is written in its own
    # language, so nothing else in this row can pair a post with its
    # translation. Without a pairing the two editions cannot declare hreflang
    # and Google reads them as competitors for one query, then picks one.
    #
    # The value is the slug of whichever edition was written first, carried by
    # every translation of it, so the key is stable and needs no second table.
    translation_key: Mapped[str | None] = mapped_column(String(200), index=True)
