...
def _build_module_docx(project_name: str, module: str, label: str, slice_: dict) -> io.BytesIO:
    ...
    # NOTHING in a document a student hands in may name the tool. This line said
    # "Module report · generated by DoThesis" and went straight into a file the
    # route itself calls teacher-ready. run_export scrubs the brand out of every
    # document it renders (results_render.scrub_sections), but this route builds
    # its docx with python-docx directly and never goes through it — so the one
    # place the invariant is enforced could not see this pipeline at all.
    doc.add_paragraph().add_run("Module report").italic = True
