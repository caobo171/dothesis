...
- Normal text (Times New Roman 13pt, 1.25 line spacing, justified)
- Block quotes
- List styles
"""
...
# One home for the body spacing so the body and every style that must match it
# cannot drift apart (they did: body at 1.25 under headings still at 2.0 is what
# makes a document look assembled rather than written).
...
def create_reference_document():
    ...
    #
    # 13pt / 1.25 / justified (2026-08-20, product owner). Replaced 12pt double-
    # spaced left-aligned. line_spacing is set as a MULTIPLE rather than through
    # line_spacing_rule because WD_LINE_SPACING has no 1.25 member — assigning a
    # float sets the rule to MULTIPLE for us, and setting the rule afterwards
    ...
    title.paragraph_format.line_spacing = BODY_LINE_SPACING
    ...
    h3.paragraph_format.line_spacing = BODY_LINE_SPACING
    ...
    block_text.font.size = Pt(13)
    block_text.paragraph_format.left_indent = Inches(0.5)
    block_text.paragraph_format.right_indent = Inches(0.5)
    block_text.paragraph_format.line_spacing = BODY_LINE_SPACING
    # Block quotes are running prose, so they justify with the body.
    block_text.paragraph_format.alignment = WD_ALIGN_PARAGRAPH.JUSTIFY
    ...
    list_bullet.font.size = Pt(13)
    list_bullet.paragraph_format.line_spacing = BODY_LINE_SPACING
    ...
    list_number.font.size = Pt(13)
    list_number.paragraph_format.line_spacing = BODY_LINE_SPACING
