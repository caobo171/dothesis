...
| **Focus is advisory, never a router or tool restriction** | The agent may choose any tool, order, and modules needed for the requested outcome. Explicit/recent artifact intent determines storage (`questionnaire→M3.instrument`, `analysis→M4.analysis_results`, `chapters→M5.final_sections`); `agent/artifact_routing.py` rejects a mismatched `commit_slice`. |
| Read = free; mutate records the latest focus + downstream `needs_review` ⚠ | `commit_slice` sets focus to the artifact's owning module and propagates over the static DAG (M1→…, M2→M3..M5, M3→M4,M5, M4→M5); only *started* modules get flagged. `read_slice` never mutates. Cross-module work is allowed when one requested outcome needs it. |
