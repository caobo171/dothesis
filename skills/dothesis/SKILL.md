...
- ❌ Do not act on a module without having read its skill this session. This is
  mechanically tracked: a commit made without a recorded read comes back with a
  `skill_reminder` field on the SUCCESSFUL result. The commit went through — read the
