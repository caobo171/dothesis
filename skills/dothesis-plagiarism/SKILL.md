...
1. The `plagiarism-check` tool (`POST /api/v1/tools/plagiarism-check`), which
   runs the deployment's configured similarity provider over a passage.
2. The citation practice that prevents a similarity problem in the first place —
...
1. **Find the passages at risk.** Text with no citation but a specific claim, a
   statistic, a definition, or a construct's operationalisation is where
   uncited borrowing lives. You can spot these by reading.
2. **Check the reference list against the in-text citations.** Sources cited but
   missing from the list, and listed but never cited, are both flags a
...
- Never produce a similarity number that did not come from a provider.
