"""Overrides autouse fixtures for unit tests in orchestrator/tests/tools/.

The top-level conftest's _bind_db fixture requires the app module which isn't
needed for pure orchestrator.tools unit tests. Tests here should use direct
...
def _bind_db(request, pg_url=None, monkeypatch=None):
    ...
    Tests in orchestrator/tests/tools/ are isolated unit tests using mocks,
